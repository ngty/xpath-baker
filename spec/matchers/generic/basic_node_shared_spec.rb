shared 'basic node matcher' do
  ...
  describe '> generating condition (w string value)' do
    ...
    before do
      ...
      @condition_should_equal = lambda do |config, expected|
        @node_matcher.new(@name, @val, XPathBaker::Configuration.new(config)).
  ...
  describe '> generating condition (w multi elements array value)' do
    ...
    before do
      ...
      @condition_should_equal = lambda do |config, expected|
        @node_matcher.new(@name, @vals, XPathBaker::Configuration.new(config)).
          condition.should.equal(expected)
      end
    end
  ...
  describe '> generating condition (with value NIL_VALUE)' do
    ...
    before do
      @name, @val = @name || :something, XPathBaker::Matchers::Matchable::NIL_VALUE
