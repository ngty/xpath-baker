...
describe "XPF::PathBuilding" do
  ...
  describe '> q (quoting string)' do
    ...
    should %\escape '"' in string\ do
      XPF.q('"A"B').should.equal %\concat("",'"',"A",'"',"B")\
    end

    should %\apply quote (with '"') in string\ do
      XPF.q("'A'B").should.equal %\"'A'B"\
    end
  ...
  describe '> t (getting node text)' do
    ...
    before do
      @set_config = lambda {|flag| XPF.config = @config_klass.new(false, false, flag) }
    end
